"""
FastAPI application for the HerAndHim Web Dashboard.
...
def create_app(provider: LLMProvider | None, *, build_provider_fn=None) -> FastAPI:
    ...
    origins = [o.strip() for o in
               os.environ.get("HERANDHIM_ALLOWED_ORIGINS", "").split(",") if o.strip()]
    if origins:
        app.add_middleware(
            CORSMiddleware,
...
def _get_agent() -> Agent | None:
    ...
    try:
        ...
        agent = PersistentAgent(
            ...
            store=SessionStore(),       # SessionStore reads HERANDHIM_HOME → per-tenant
...
async def _api_config_save(request: Request):
    """Save new configuration to herandhim.json and hot-reload the provider.
    ...
    cfg_path = config.config_path()
    if cfg_path is None:
        cfg_path = config.HERANDHIM_HOME / "herandhim.json"
...
async def _api_skills():
    ...
    if agent is None:
        try:
            ...
            skills_dirs = [pkg_templates, os.path.join(str(config.HERANDHIM_HOME), "context", "skills")]
...
async def _api_status():
    ...
    # Per-tenant session store: SessionStore() reads HERANDHIM_HOME, which is
...
async def _api_save_soul(request: Request):
    ...
    try:
        ...
        soul_dir = config.HERANDHIM_HOME / "context" / "soul"
...
async def _api_save_persona(request: Request):
    ...
    try:
        ...
        persona_dir = config.HERANDHIM_HOME / "context" / "persona"
...
async def _api_get_tools_notes():
    ...
    tools_dir = config.HERANDHIM_HOME / "context" / "tools"
    content = None
    if tools_dir.is_dir():
...
async def _api_get_index():
    ...
    index_path = config.HERANDHIM_HOME / "context" / "memory" / "INDEX.md"
    content = ""
...
async def _api_save_index(request: Request):
    ...
    try:
        body = await request.json()
        content = body.get("content", "").strip()
        index_dir = config.HERANDHIM_HOME / "context" / "memory"
